...
public class NotTest {
    ...
    @Test
    public void NotWithValidEqualityPasses() {
    ...
    @Test
    public void nestedNotWithValidEqualityPasses() {
        String input = "board(3,3); player red has 2 piece knight; assert test (!!count(knight) == count(knight));";
        BoardGameLangParser parser = ParseTreeHelper.createParser(input);
        ProgramNode program = (ProgramNode) new AstBuilder().visit(parser.program());
    ...
    @Test
    public void notWithOccupiedPasses() {
        String input = "board(3,3); assert test (!occupied((1,1)));";
        BoardGameLangParser parser = ParseTreeHelper.createParser(input);
        ProgramNode program = (ProgramNode) new AstBuilder().visit(parser.program());
